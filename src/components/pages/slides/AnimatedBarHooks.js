...
const Rect = ({ index, data, prev, next, x, y, height, top, bottom, sort }) => {
    ...
    const shouldUpdate = !sort && prev[index].index === data.index && prev[index].value !== data.value

    const interpolatorX = d3.interpolate(x(sort ? prevIndex : prevIndex), x(sort ? data.index : data.index))

    const interpolatorY = d3.interpolate(y(shouldUpdate ? prev[index].value : data.value), y(data.value))
    ...
    return (
        ...
        >
            ...
            <animated.text
                ...
                fontSize="10"
            >
                {shouldUpdate ? animatedProps.t.interpolate(t => format(interpolator(t).value)) : format(data.value)}
            </animated.text>
        </animated.g>
    )
}
...
const Bar = props => {
    ...
    data.current = sort ? [...props.data].sort((a, b) => b.value - a.value) : [...props.data]
